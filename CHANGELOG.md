...
- Extend the wheel/sdist Schema distribution release gate and installed-package smoke workflows to recognize all nineteen public Schemas and validate the registered Observation, World State, State Transition, Verification Session, Discrepancy Report, Correction Request, and Core Benchmark Artifacts.
- Reframe the architecture documentation from a single execution pipeline into four explicit planes: perception/open reasoning, explicit spatiotemporal world state, verification/state evolution, and control/real-world action. The document now distinguishes the implemented Observation, World State, State Transition, Verification Session, Discrepancy Report, and Correction Request foundations from planned Observation merging, automatic diff, successor-state materialization, executable impact, provider, and reevaluation abstractions.
