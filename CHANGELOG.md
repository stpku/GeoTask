...
- Add GT25 as a scenario-first bounded safety-distance recompute case. A fictional UAV moves from corridor chainage 100 to 130 metres while fixed obstacles remain at 150 and 260 metres. A case-specific builder binds the position Observation, World State, GeoTask Document, Discrepancy Report, and Correction Request to a Recompute Derivation Result that uses only the allowlisted `subtract` method, deriving 20- and 130-metre UAV distances while preserving 110-metre fixed-facility spacing and 48% battery as immutable reusable paths. Tests reject incomplete or overlapping scope, unregistered methods, and exact-byte tampering, while preserving that Core does not discover dependencies automatically, execute arbitrary code, materialize a successor state, rerun safety checks, release outputs, establish external truth, authorize action, or execute action.
- Add GT26 as a scenario-first bounded flight-service-station schedule correction case. A fictional notice narrows operating hours from 08:00–22:00 to 09:00–18:00, making a 20:30 mission availability result stale. A case-specific builder binds the notice Observation and World State to a Discrepancy Report and one-path Correction Request, preserves location code, radio frequency, service types, and contact channel as immutable paths, and blocks the mission output and dispatch action until a valid successor state and completed recheck exist. Tests reject expanded or overlapping correction scope, replacement values that differ from the notice, missing recheck criteria, and exact-byte tampering, while preserving that Core does not fetch or compare real notices, apply correction, materialize state, rerun missions, release outputs, establish external truth, authorize dispatch, or execute action.
- Add GT27 as a scenario-first weather-triggered incremental reevaluation case. A fictional east-zone wind Observation raises the operating value from 6 to 12 m/s. Revision 7 absorbs the new weather value while retaining stale mission results; revision 8 reevaluates only Missions A and D in the matching region and active time window. Mission A changes from suitable to unsuitable, Mission D remains suitable after explicit recheck, and Missions B and C remain immutable reusable results. The case binds World States, Discrepancy Report, Correction Request, Impact Graph, execution result, and Incremental Reevaluation Result with two assertion targets and two output-release targets. Tests reject scope expansion, immutable-result changes, and exact-byte tampering while preserving that Core does not discover dependencies, fetch real weather, run a generic weather operator, publish production outputs, or authorize flight.
- Add GT28 as a scenario-first automatic-takeoff authorization-gate case. Route, altitude, forecast-window, and wind checks pass, while airspace, operator, departure-site, weather-release, and mission authorization identifiers remain unknown. A fixed Control Evaluation keeps automatic takeoff authorization and the takeoff command blocked, while Verification Session records the route-weather precheck as eligible. Tests verify exact-byte bindings, reject authorization inference, and prove that even a fully true authorization context only makes outputs eligible while `action_executed` remains false; Core does not contact real authorities, establish external authorization truth, publish outputs, send commands, or execute flight.
- Add Verification Provider Profile v0.1 with four registered public Artifacts: Provider Descriptor, Verification Request, Verification Response, and Assurance Profile. Strict loaders, exact Request/Descriptor byte bindings, anti-self-assurance rules, independent-group, freshness, reproducibility, calibration, and action-boundary checks are exposed through read-only `geotask provider inspect`, `check`, and `validate` commands.
- Add GT29 as a fictional multi-source weather conflict case. A mock weather service reports 8 m/s while an onsite sensor reports 13 m/s against a 12 m/s mission limit. Both responses are fresh and independently grouped, but Assurance remains `unknown`; source precedence is not inferred, values are not averaged, weather and takeoff outputs remain blocked, and the next action requests a third independent source.
- Add GT30 as a fictional three-source weather conflict case. A mobile lidar source independently reports 13 m/s, matching the onsite sensor while the mock weather service remains at 8 m/s. The exact-consensus Assurance Profile accepts all three Providers as fresh, reproducible, calibrated, and independent, but rejects implicit majority voting, preserves the minority response, keeps outputs and actions blocked, and requests explicit weather adjudication.
- Add GT31 as a fictional human weather adjudication case. The human-review Request binds all three GT30 responses, the GT30 Assurance artifacts, and one scoped context-evidence packet. The review preserves every raw result, records that two 13 m/s readings are local-test-flow observations outside the mission-corridor ambient-wind claim, and produces a bound 8 m/s response. A separate Assurance evaluation makes `weather_condition_verified` eligible, while the existing GT28 Control Evaluation keeps automatic takeoff authorization and the takeoff command blocked by five missing authorizations; no Provider self-assurance, production release, authorization, command emission, or action execution is inferred.
- Add separate Chinese and English project homepages plus Chinese and English terminology guides. Human-facing prose follows one language per entry point while machine identifiers, commands, filenames, and Artifact IDs remain stable English contracts.
