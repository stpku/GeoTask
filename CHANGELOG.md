...
- Add `geotask.verification-session` v0.1 as an immutable audit snapshot binding one World State semantic fingerprint to exact serialized task, execution-result, control-evaluation, State Transition, and discrepancy artifacts. It records session outcome, action eligibility, and recheck triggers, provides exact-byte SHA-256 binding validation, and keeps Session structure, binding verification, linked-artifact semantic validation, and operational execution as separate layers. The Registry now exposes sixteen Artifacts and the offline Schema Bundle contains seventeen Schemas.
...
- Extend the wheel/sdist Schema distribution release gate and installed-package smoke workflows to recognize all seventeen public Schemas and validate the registered Observation, World State, State Transition, Verification Session, and Core Benchmark Artifacts.
- Reframe the architecture documentation from a single execution pipeline into four explicit planes: perception/open reasoning, explicit spatiotemporal world state, verification/state evolution, and control/real-world action. The document now distinguishes the implemented Observation, World State, State Transition, and Verification Session foundations from planned Observation merging, automatic diff, discrepancy, impact, provider, and reevaluation abstractions.
