...
- Extend the wheel/sdist Schema distribution release gate and installed-package smoke workflows to recognize all fifteen public Schemas and validate the registered Observation, World State, and Core Benchmark Artifacts.
- Reframe the architecture documentation from a single execution pipeline into four explicit planes: perception/open reasoning, explicit spatiotemporal world state, verification/state evolution, and control/real-world action. The document now distinguishes the implemented Observation and World State foundations from planned Observation merging, `StateTransition`, `VerificationSession`, discrepancy, impact, provider, and reevaluation abstractions.
