...
GeoTask连接多模态大模型的开放推理与本地时空验证框架。它把模型提出的对象、观察、断言、方案和候选动作转化为显式任务，检查是否遗漏约束、是否违反几何与时间事实、证据是否仍然有效，并在结论不成立时给出可定位的阻断、补证或修订路径。

- **多模态模型负责理解与提出：** 从文本、地图、图像、视频和状态数据中形成观察、假设与方案；
- **GeoTask Core负责结构化与验证：** 检查对象、坐标、单位、证据、算子契约和本地确定性结果；
- **控制层负责纠偏与门控：** 标记矛盾、要求补证、限制修订范围，并决定输出是否可继续使用；
- **Runtime与Domain Pack负责连接现实：** 接入权威数据、行业规则、本地模型、人工复核和生产动作。

> **技术定义仍然是：** GeoTask是一种面向AI智能体的可验证时空任务协议。协议是实现形式，验错、纠偏与行动门控是核心价值。
...
多模态大模型越来越会看懂场景、调用工具和生成计划。能力提升后，稀缺问题从“能不能给出一个时空答案”转向：

- 生成过程是否遗漏了坐标、时间、高度、能力或安全余量约束；
- 结果是否符合几何、拓扑、时间和物理事实；
- 证据更新后，原结论是否仍然成立；
- 错误能否定位到具体命题、对象或允许修订的字段；
- 未经验证的结论是否会越过门控进入现实行动。

一次Tool Calling可以完成局部函数调用，却不会自动保留完整任务、证据状态、受影响输出、修订边界和恢复条件。GeoTask把这些信息组织为可验证、可审计的任务与Artifact：

```mermaid
flowchart LR
  M[多模态模型提出观察与方案] --> T[GeoTask结构化任务]
  T --> V[本地时空验证]
  V --> S{验证状态}
  S -->|verified| G[行动门控]
  S -->|contradicted| C[限定范围纠偏]
  S -->|unverifiable| E[补充证据]
  C --> T
  E --> T
  N[新状态到来] --> V
```

当前公共Core已经实现任务结构化、确定性验证、证据与控制状态、Agent机械修复和限定路径重试。统一`VerificationSession`、通用差异报告、影响图和增量重算仍属于后续路线图。
