...
- [GeoTask Artifact Registry v1.0](spec/geotask-artifact-registry-v1.0.md) — `geotask inspect schemas` discovery for all thirty-one public Artifacts and thirty-two public JSON Schemas.
...
- [GeoTask Trajectory and Moving Object Profile v0.1](spec/geotask-trajectory-profile-v0.1.md) — identity/position separation, strictly ordered timezone-aware samples, adjacent-segment metrics, caller-declared classifications, identity candidates, adjudication, review-only merge proposals, approval records, bounded object-graph change requests, and the GT33–GT41 non-prediction/non-execution boundary.
...
- [GeoTask Artifact Validation v1.0](spec/geotask-artifact-validation-v1.0.md) — one `geotask artifact validate` entry point for validating all thirty-one registered public Artifacts, including trajectory identity adjudication, identity merge proposals, approval records, object-graph change requests, world-state, Agent, Runtime, Verification Provider, benchmark, and validation-report contracts.
