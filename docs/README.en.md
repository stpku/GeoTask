...
- [GeoTask World State v0.1](spec/geotask-world-state-v0.1.md) — one versioned snapshot of world objects, attributes, relations, validity, uncertainty, and closed Observation/Evidence references, without merging Observations or computing a State Transition.
- [GeoTask Artifact Registry v1.0](spec/geotask-artifact-registry-v1.0.md) — `geotask inspect schemas` discovery for all fourteen public Artifacts, their schemas, versions, and operating commands.
- [GeoTask Artifact Validation v1.0](spec/geotask-artifact-validation-v1.0.md) — one `geotask artifact validate` entry point for validating all fourteen registered public Artifacts, including Observation, World State, Agent reports, Runtime messages, the Core benchmark report, and validation reports themselves, by stable Artifact ID.
