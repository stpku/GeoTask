...
- [GeoTask Artifact Registry v1.0](spec/geotask-artifact-registry-v1.0.md) — `geotask inspect schemas` discovery for all sixteen public Artifacts, their schemas, versions, and operating commands.
- [GeoTask Artifact Validation v1.0](spec/geotask-artifact-validation-v1.0.md) — one `geotask artifact validate` entry point for validating all sixteen registered public Artifacts, including Observation, World State, State Transition, Verification Session, Agent reports, Runtime messages, the Core benchmark report, and validation reports themselves, by stable Artifact ID.
