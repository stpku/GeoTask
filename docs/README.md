...
- [Observation v0.1](spec/geotask-observation-v0.1.md)：首个世界模型Artifact，用于表达带来源、时间、生产者和不确定性的世界命题，但不宣称命题真实，也不更新WorldState。
- [制品注册表v1.0](spec/geotask-artifact-registry-v1.0.md)：通过`geotask inspect schemas`统一发现13类公共Artifact的Schema、版本及操作命令。
- [统一制品校验v1.0](spec/geotask-artifact-validation-v1.0.md)：通过`geotask artifact validate`按稳定Artifact ID校验13类公共制品，包括Observation、Agent报告、Runtime消息、Core基准报告与验证报告自身，并输出统一文本/JSON报告。
