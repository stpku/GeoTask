...
- [制品注册表v1.0](spec/geotask-artifact-registry-v1.0.md)：通过`geotask inspect schemas`统一发现18类公共Artifact的Schema、版本及操作命令。
- [统一制品校验v1.0](spec/geotask-artifact-validation-v1.0.md)：通过`geotask artifact validate`按稳定Artifact ID校验18类公共制品，包括Observation、World State、State Transition、Verification Session、Discrepancy Report、Correction Request、Agent报告、Runtime消息、Core基准报告与验证报告自身，并输出统一文本/JSON报告。
