...
- `inspect schemas` lists all twenty-eight public Artifacts, covering the GeoTask
  ...
  verifies the Registry Schema and all twenty-eight Artifact Schemas; with an
...
- `schema verify` checks the versioned Bundle Manifest, expected filenames,
  ...
  With no Artifact ID it checks all twenty-eight bundled Schemas; supplying one
...
The ten fixed fictional cases cover all fourteen public deterministic operators,
including discrete trajectory duration, adjacent-segment metrics, caller-declared segment classifications, bounded scalar acceleration estimates, and boundary-sample identity candidates, strict Result round trips,
