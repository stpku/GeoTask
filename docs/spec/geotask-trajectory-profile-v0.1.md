...
Reference cases: GT33–GT42
Scope: discrete observations, adjacent-sample metrics, caller-declared classifications, bounded scalar acceleration estimates, boundary-sample identity candidates, exact-bound external identity adjudication, review-only identity-merge proposals, non-executing approval records, bounded object-graph change requests, and non-applying application approval records
...
GT42 adds the registered `geotask.object-graph-change-application-approval-record` Artifact. It binds the exact GT41 request and requires one explicit `approved`, `rejected`, or `evidence_required` decision for every caller-declared application-approval role. Rejection takes precedence; otherwise evidence requests keep application blocked; only all-role approval makes a later bounded application Artifact eligible. Even then, `application_authorized`, reference mutation, object-graph mutation, World State update, publication, authorization, and execution remain false. See [Object Graph Change Application Approval Record v0.1](geotask-object-graph-change-application-approval-record-v0.1.md).
...
- GT42 receives a non-review-ready GT41 request, omits or duplicates a caller-declared application-approval role, accepts an undeclared role or decision, records `evidence_required` without an evidence reference, derives an aggregate decision inconsistent with role decisions, or claims that approval authorized or applied the change, rewrote references, changed the object graph or World State, published, authorized, or executed an update.
