...
多模态大模型正在从“回答问题”走向“理解场景、生成计划并影响现实行动”。随着模型越来越会看、会规划、会调用工具，最稀缺的能力不再只是生成一个时空结论，而是确认生成过程没有遗漏约束、结果没有违反几何与时间事实、证据更新后结论仍然成立、错误能够被定位和修正，以及未经验证的输出不会直接进入现实行动。模型能力越强、行动链越长，这一验证缺口越重要。

GeoTask 将自身定位为连接多模态大模型开放推理与本地时空验证框架的验错、纠偏和行动门控层。它把模型提出的观察、对象、断言、方案与候选动作转化为显式时空任务，通过本地确定性算子、证据与控制合同复算可计算命题，区分 `verified`、`contradicted`、`unverifiable` 和阻断状态，并为补证、限定范围修订、重新执行和外部授权保留机器可读路径。模型可以负责理解、分解、解释和提出动作，但不能仅凭语言流畅度宣布计算已经正确。

“可验证时空任务协议”仍然是 GeoTask 的技术实现形式。公共 Core 提供任务与 Artifact 契约、本地确定性验证、证据绑定、控制评估和 Agent 修订基础；Runtime 与 Domain Pack 负责权威数据、行业规则、本地模型、人工复核和生产动作。GeoTask 不替代 GIS、工作流引擎或审批系统，而是让这些系统与开放推理之间形成可读、可执行、可验证、可纠偏、可审计的安全连接。
...
```text
过去：专用时空模型直接给答案
现在：大模型理解场景并提出答案，本地时空框架辅助与验算
未来：开放推理持续生成方案，本地验证持续纠偏并守住行动边界
```

模型能力越强，系统越需要回答以下问题：

- 生成过程是否遗漏坐标、边界、时间、高度、对象能力或资源余量；
- 结果是否符合几何、拓扑、时间和物理事实；
- 新证据或新状态到来后，原结论哪些仍然有效、哪些必须失效；
- 错误能否定位到具体命题、对象、证据或允许修改的字段；
- 未经验证的结论是否会进入设备运动、任务审批或风险处置。

因此，多模态能力提升不会削弱 GeoTask 的价值，反而会把其作用从“给模型提供一个空间工具”提升为“连接开放推理与现实行动的时空验证层”。
...
```text
Multimodal Model / Agent Proposal
              ↓
   GeoTask Task and Artifact Contracts
              ↓
Local Verification → Status / Discrepancy / Evidence Request
              ↓                         ↑
Bounded Correction / Recheck ← New Evidence or State
              ↓
        Action Eligibility Gate
```

当前公共 Core 已实现上述循环中的任务结构化、本地确定性验证、证据与控制状态、Agent 机械修复、限定路径重试和证据恢复。统一 Verification Session、通用差异报告、影响图和增量重算仍是后续工程目标。
...
> **面向智能体的可验证时空任务协议。它使用统一的对象、算子、断言、执行计划、证据和输出契约描述时空任务，使模型推理能够与本地验证分离，并支持结果复算、证据治理、冲突保留、限定纠偏、任务阻断和条件恢复。**

产业定位回答“为什么需要 GeoTask”，技术定义回答“GeoTask 以什么形式实现”。这一概念包含四个关键词。
...
GeoTask 不负责提供完整地图、原始多模态识别、设备控制、行业审批或商业调度。它定义这些系统之间如何表达和交换任务、观察、断言、证据、执行结果和治理状态，并为Runtime与Domain Pack保留明确扩展边界。
