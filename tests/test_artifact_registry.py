...
def test_registry_contains_exactly_thirty_stable_public_artifacts() -> None:
    ...
    assert artifact_registry_payload()["artifact_registry"]["artifact_count"] == 30
...
def test_schema_bundle_manifest_matches_authoritative_schema_bytes() -> None:
    ...
    assert manifest["schema_count"] == 31
    assert len(manifest["schemas"]) == 31
...
def test_schema_bundle_verification_supports_all_and_one_artifact() -> None:
    ...
    assert all_report["checked_count"] == 31
...
def test_registry_generation_and_validation_guidance_is_explicit() -> None:
    ...
    impact_graph = get_artifact_descriptor("geotask.impact-graph")
    incremental_result = get_artifact_descriptor(
        "geotask.incremental-reevaluation-result"
    )
    ...
    observation_merge_result = get_artifact_descriptor(
        "geotask.observation-merge-result"
    )
    ...
    assert state_transition.generation_command is None
    assert state_transition.wrapper_key == "state_transition"
    assert state_transition.schema_id == STATE_TRANSITION_SCHEMA_ID
    assert state_transition.schema_version == "0.1"
    assert "does not compare snapshots" in state_transition.execution_boundary
    assert "authorize action" in state_transition.execution_boundary

    assert verification_session.generation_command is None
    assert verification_session.wrapper_key == "verification_session"
    assert verification_session.schema_id == VERIFICATION_SESSION_SCHEMA_ID
    assert verification_session.schema_version == "0.1"
    assert "does not validate linked artifact semantics" in (
        verification_session.execution_boundary
    )
    assert "run rechecks" in verification_session.execution_boundary

    assert discrepancy_report.generation_command is None
    assert discrepancy_report.wrapper_key == "discrepancy_report"
    assert discrepancy_report.schema_id == DISCREPANCY_REPORT_SCHEMA_ID
    ...
    assert impact_graph.generation_command is None
    assert impact_graph.wrapper_key == "impact_graph"
    assert impact_graph.schema_id == IMPACT_GRAPH_SCHEMA_ID
    assert impact_graph.schema_version == "0.1"
    assert "does not discover impact" in impact_graph.execution_boundary
    assert "authorize actions" in impact_graph.execution_boundary

    assert incremental_result.generation_command is None
    assert incremental_result.wrapper_key == "incremental_reevaluation_result"
    assert incremental_result.schema_id == INCREMENTAL_REEVALUATION_RESULT_SCHEMA_ID
    ...
    assert observation_merge_result.generation_command is None
    assert observation_merge_result.wrapper_key == "observation_merge_result"
    assert observation_merge_result.schema_id == OBSERVATION_MERGE_RESULT_SCHEMA_ID
    assert observation_merge_result.schema_version == "0.1"
    assert "does not create" in observation_merge_result.execution_boundary
    assert "compute a State Transition" in observation_merge_result.execution_boundary
...
def test_public_manifest_requires_artifact_registry_assets() -> None:
    ...
    for path in (
        ...
        "docs/spec/geotask-observation-merge-result-v0.1.md",
        ...
        "examples/core/impact_graph_uav_recheck.json",
        "examples/core/world_state_uav_separation_successor.json",
        ...
        "examples/core/observation_merge_result_uav_recheck.json",
        "examples/core/world_state_uav_separation_observation_merged.json",
        ...
        "schemas/geotask-observation-merge-result-v0.1.schema.json",
        ...
        "tests/v1/test_observation_merge_v0_5.py",
    ):
...
def test_schema_verify_supports_text_json_and_exact_artifact() -> None:
    ...
    assert "Schema Bundle valid: 31 schema(s), version 1.0" in text_result.stdout
    assert text_result.stdout.count("sha256=") == 31
...
def test_inspect_schemas_default_yaml_is_parseable() -> None:
    ...
    assert registry["artifact_count"] == 30
...
def test_inspect_schemas_can_include_bundle_integrity_results() -> None:
    ...
    assert all_payload["artifact_registry"]["artifact_count"] == 30
    all_verification = all_payload["schema_bundle_verification"]
    assert all_verification["valid"] is True
    assert all_verification["checked_count"] == 31
...
def test_inspect_schemas_verify_emits_json_before_nonzero_exit(
    ...
    assert payload["artifact_registry"]["artifact_count"] == 30
