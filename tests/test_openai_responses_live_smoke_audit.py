...
def _write_verified_closure(
    ...
    closure_path = tmp_path.parent / f"{tmp_path.name}-verified-closure.json"
...
def test_closure_verification_reanchors_digest_and_source_evidence(
    tmp_path: Path,
) -> None:
    (
        ticket_path,
        ...
        authorization_id,
        closure_sha256,
    ) = _write_verified_closure(tmp_path)
    before = {
    ...
    original = {name: sys.modules.pop(name, None) for name in provider_modules}
    try:
        result = AUDIT.verify_closure_manifest(
            ticket_path,
            claim_path,
            report_path,
            closure_path,
...
def test_closure_verification_rejects_wrong_or_malformed_digest(
    tmp_path: Path,
) -> None:
    ...
    mismatch = AUDIT.verify_closure_manifest(
        ticket,
        claim,
        report,
        closure,
    ...
    malformed = AUDIT.verify_closure_manifest(
        ticket,
        claim,
        report,
        closure,
...
def test_rehashed_closure_tampering_still_fails_evidence_and_time_binding(
    ...
    SMOKE._write_report(closure, payload)
    changed_digest = hashlib.sha256(closure.read_bytes()).hexdigest()
    changed = AUDIT.verify_closure_manifest(
        ticket,
        claim,
        report,
        closure,
    ...
    SMOKE._write_report(closure, payload)
    changed_digest = hashlib.sha256(closure.read_bytes()).hexdigest()
    time_invalid = AUDIT.verify_closure_manifest(
        ticket,
        claim,
        report,
        closure,
    ...
    SMOKE._write_report(closure, payload)
    changed_digest = hashlib.sha256(closure.read_bytes()).hexdigest()
    contract_invalid = AUDIT.verify_closure_manifest(
        ticket,
        claim,
        report,
        closure,
...
def test_closure_verification_rejects_non_strict_json(tmp_path: Path) -> None:
    ticket, claim, report, closure, _authorization_id, _digest = (
        _write_verified_closure(tmp_path)
    )
...
def test_closure_verification_detects_changed_source_evidence(
    tmp_path: Path,
) -> None:
    ...
    SMOKE._write_report(report, report_payload)
...
def test_closure_verification_detects_midflight_source_evidence_change(
    ...
    original_verify = CLOSURE_VERIFIER.verify_evidence_bundle
    call_count = 0
...
def test_closure_verification_rejects_repository_path_or_open_permissions(
    ...
    inside = tmp_path / "retained-closure.json"
    inside.write_bytes(closure.read_bytes())
    if os.name != "nt":
        inside.chmod(0o600)
    inside_result = AUDIT.verify_closure_manifest(
        ticket,
        claim,
        report,
        inside,
    ...
    if os.name != "nt":
        closure.chmod(0o644)
        permission_result = AUDIT.verify_closure_manifest(
            ticket,
            claim,
            report,
            closure,
...
def test_private_auditor_and_tests_are_excluded_from_public_export() -> None:
    ...
    assert "examples/runtime/openai_responses_live_smoke_closure_verifier.py" not in exported
