...
def test_root_page_is_project_portal_not_gt01_experience() -> None:
    ...
    assert "Observation v0.1、World State v0.1、State Transition v0.1、Verification Session v0.1、Discrepancy Report v0.1、Correction Request v0.1、Impact Graph v0.1、来源绑定的受限重算值推导、受限后继状态物化、Incremental Reevaluation Result v0.1" in html
    assert "自动差异计算、Observation合并、Impact Graph自动发现与传播执行以及通用推导方法扩展" in html
